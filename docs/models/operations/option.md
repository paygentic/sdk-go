...
```go
paygentic.WithServerURL("https://api.example.com")
```
...
```go
paygentic.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
paygentic.WithClient(httpClient)
```
...
```go
paygentic.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
paygentic.WithTimeout(30 * time.Second)
```
